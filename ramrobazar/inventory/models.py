...
from ramrobazar.account.models import User, UserProfile
...
class Product(models.Model):
    ...
    is_blocked = models.BooleanField(default=False, verbose_name=_("product blocked"), help_text=_("format: true->product is blocked"))
    ...
    users_wishlist = models.ManyToManyField(User, related_name='user_wishlist_product', blank=True)
...
class Service(models.Model):
    web_id = models.CharField(max_length=50, unique=True, verbose_name=_("service web id"), help_text=_("format: required, unique"))
    slug = models.SlugField(max_length=255, null=False, blank=False, verbose_name=_("service url"), help_text=_("format: required, letters, numbers, underscore or hyphen"))
    name = models.CharField(max_length=250, null=False, blank=False, verbose_name=_("service name"), help_text=_("format: required, max_length=250"))
    seller = models.ForeignKey(User, related_name="services", on_delete=models.PROTECT)
    description = models.TextField(verbose_name=_("service description"), help_text=_("format: required"))
    category = TreeManyToManyField(Category)
    ...
    created_at = models.DateTimeField(auto_now_add=True, editable=False, verbose_name=_("date service created"), help_text=_("format: Y-m-d H:M:S"))
    updated_at = models.DateTimeField(auto_now=True, verbose_name=_("date service last updated"), help_text=_("format: Y-m-d H:M:S"))
    users_wishlist = models.ManyToManyField(User, related_name='user_wishlist_service', blank=True)
    reported_by = models.ManyToManyField(User, related_name='reported_service', blank=True)
...
class Media(models.Model):
    product = models.ForeignKey(Product, null=True, blank=True, related_name="media", on_delete=models.PROTECT)
    service = models.ForeignKey(Service, null=True, blank=True, related_name="media", on_delete=models.PROTECT)
